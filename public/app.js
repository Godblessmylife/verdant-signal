(function () {
  ...
  function updateRuntimeDom(game) {
    ...
    if (game === 'aviator') {
      ...
      if (caption) caption.textContent = runtime.phase === 'countdown' ? `${t('countdown')}: ${runtime.countdown}` : t('simulationMode');
  ...
  function renderAuth() {
    ...
    app.innerHTML = `<section class="auth-visual" aria-hidden="true"><div class="brand-lockup"><span class="brand-mark">V</span><span><strong>VERDANT</strong><small>SIGNAL / WORKSPACE</small></span></div><div class="auth-visual-copy"><p class="eyebrow">${t('appKicker')}</p><h1>${t('signInTitle')}</h1><p>${t('signInCopy')}</p></div><div class="signal-readout"><div><span>${t('mode')}</span><strong>${t('simulation')}</strong></div><div><span>${t('games')}</span><strong>05</strong></div></div></section><section class="auth-card-wrap"><div class="auth-card"><div class="brand-lockup"><span class="brand-mark">V</span><span><strong>VERDANT</strong><small>SIGNAL</small></span></div><p class="eyebrow">${t('appKicker')}</p><h2>${t('signInTitle')}</h2><p class="lede">${t('signInCopy')}</p><form class="stack-form" id="auth-form"><label for="player-id">${t('playerId')}<input id="player-id" name="playerId" autocomplete="username" placeholder="${t('playerIdPlaceholder')}" minlength="3" maxlength="64" required></label><label for="access-code">${t('accessCode')}<input id="access-code" name="accessCode" type="password" autocomplete="current-password" placeholder="${t('accessCodePlaceholder')}" minlength="6" maxlength="128" required></label><button class="button button-primary button-full" type="submit" ${state.busy ? 'disabled' : ''}>${state.busy ? t('signingIn') : t('signIn')}</button><p class="form-message" role="alert" aria-live="polite">${escapeHTML(state.message)}</p><p class="helper">${t('newSession')}</p></form>${legalLinks()}</div></section>${legalDialog()}`;
  ...
  function gameCards() {
    return `<section class="games-section"><div class="section-heading"><div><p class="panel-kicker">${t('games')} / 05</p><h2>${t('chooseInstrument')}</h2></div><span class="section-count">${t('liveDemo')}</span></div><div class="game-card-grid">${games.map((game, index) => `<button class="game-card game-card-${game} ${game === state.activeGame ? 'active' : ''}" type="button" data-game="${game}"><span class="game-card-index">0${index + 1}</span><strong>${gameLabel(game)}</strong><span><i class="game-dot"></i>${t('demoAnalysis')}</span></button>`).join('')}</div></section>`;
  }
  ...
  function renderAviatorStage() {
    ...
    return `<div class="visual-stage aviator-stage pipeline-stage" data-runtime-game="aviator" data-phase="${phase}"><canvas data-game-canvas="aviator" aria-label="${t('flightPath')}"></canvas><div class="aviator-grid-labels" aria-hidden="true"><span>6.00x</span><span>4.00x</span><span>2.00x</span><span>1.00x</span></div><div class="aviator-center-readout"><span class="readout-label">${t('currentMultiplier')}</span><strong data-runtime-multiplier>${escapeHTML(multiplier)}</strong><span class="round-state" data-runtime-state aria-live="polite">${phaseLabel(phase)}</span><span class="aviator-countdown" data-runtime-countdown>${phase === 'countdown' ? `${t('countdown')}: ${countdown}` : ''}</span></div><div class="aviator-axis"><span>0s</span><span>${t('roundTrace')}</span><span>12s</span></div><div class="stage-caption"><span>${t('flightPath')}</span><span data-runtime-caption>${phase === 'countdown' ? `${t('countdown')}: ${countdown}` : t('simulationMode')}</span></div></div>`;
  }
  ...
  function renderMinesStage() {
    ...
    return `<div class="visual-stage mines-stage pipeline-stage" data-runtime-game="mines" data-mine-size="${size}">${board}<div class="mine-readout"><span>${t('recommendedCells')} / ${mines}</span><strong data-runtime-caption>${phaseLabel(runtime.phase)}</strong></div><div class="stage-caption"><span>${t('simulationMode')}</span><span>${t('analysisEngine')}</span></div></div>`;
  }
  ...
  function renderFootballStage() {
    ...
    return `<div class="visual-stage football-stage pipeline-stage ${runtime.phase !== 'ready' ? `is-${runtime.phase}` : ''}" data-runtime-game="football-penalties"><div class="goal-scene"><div class="goal-net"><div class="goal-grid" aria-hidden="true"></div><canvas class="scene-animation-canvas football-animation-canvas" data-animation-canvas="football" aria-hidden="true"></canvas><div class="goal-zones">${[1, 2, 3, 4, 5].map((zone) => `<button class="goal-zone ${chosen === zone ? 'is-selected' : ''}" type="button" data-game-action="${runtime.role === 'striker' ? 'football-zone' : 'football-direction'}" data-zone="${zone}" ${runtime.phase !== 'ready' ? 'disabled' : ''} aria-label="${t('zone')} ${zone}"><span>${zone}</span></button>`).join('')}</div><div class="keeper-actor" data-art-layer="keeper"><img class="keeper-sprite" src="${ART.football.goalkeeper}" alt="${t('goalkeeper')}"></div><div class="football-ball" data-art-layer="ball" aria-hidden="true"><img src="${ART.football.ball}" alt=""></div></div></div><div class="football-readout"><span class="readout-label">${t('result')}</span><strong data-football-result aria-live="polite">${runtime.result ? phaseLabel(runtime.result) : t('chooseZone')}</strong><small>${terminal && runtime.result === 'goal' ? `${t('simulatedReturn')}: 2x · ${formatMoney(runtime.stake || 100, { currencyFractionDigits: 0, currencyCode: 'RUB' })}` : t('virtualOnly')}</small></div><div class="stage-caption"><span>${runtime.role === 'striker' ? t('striker') : t('keeper')} / ${t('shotZones')}: 5</span><span>${t('demoAnalysis')}</span></div></div>`;
  }
  ...
  function createAppleRuntime() { return { game: 'apple-of-fortune', token: Date.now(), phase: 'ready', activeRow: 1, revealed: null, revealedCells: {}, animationStartedAt: 0, timers: new Set() }; }
  function revealApple(cell) { const runtime = runtimeFor('apple-of-fortune') || createAppleRuntime(); if (runtime.phase === 'opening' || runtime.phase === 'ended') return; const rows = state.analysis?.game === 'apple-of-fortune' ? state.analysis.rows : Array.from({ length: 4 }, (_, index) => ({ level: index + 1, recommendedCell: index + 2 })); const row = rows.find((item) => item.level === runtime.activeRow) || rows[0]; runtime.revealed = { level: runtime.activeRow, cell: Number(cell), safe: Number(cell) === Number(row.recommendedCell) }; runtime.animationStartedAt = performance.now(); runtime.phase = 'opening'; state.runtime = runtime; render(); scheduleRuntime(() => { runtime.revealedCells[runtime.revealed.level] = runtime.revealed; if (runtime.revealed.safe && runtime.activeRow < rows.length) { runtime.activeRow += 1; runtime.phase = 'safe'; } else runtime.phase = 'ended'; runtime.revealed = null; render(); }, 560); }

  function createMineRuntime() { return { game: 'mines', token: Date.now(), phase: 'ready', size: Number(document.getElementById('mine-size')?.value || 25), mines: Number(document.getElementById('mine-count')?.value || 4), opened: {}, opening: null, previewResult: null, animationStartedAt: 0, timers: new Set() }; }
  function resetMineFromControls() { stopGameAnimation(); state.runtime = createMineRuntime(); render(); }
  function openMine(cell) { const runtime = runtimeFor('mines') || createMineRuntime(); if (runtime.phase === 'opening' || runtime.phase === 'mine' || runtime.opened[cell]) return; const recommended = state.analysis?.game === 'mines' ? state.analysis.recommendedCells : [0, 2, 7]; runtime.opening = Number(cell); runtime.previewResult = recommended.includes(Number(cell)) ? 'safe' : 'mine'; runtime.animationStartedAt = performance.now(); runtime.phase = 'opening'; state.runtime = runtime; render(); scheduleRuntime(() => { const result = runtime.previewResult; runtime.opened[cell] = { result }; runtime.opening = null; runtime.phase = result === 'mine' ? 'mine' : 'safe'; render(); }, 460); }

  function createFootballRuntime() { return { game: 'football-penalties', token: Date.now(), phase: 'ready', role: 'striker', selectedZone: 3, selectedDirection: 3, stake: 100, shotZone: 3, result: null, animationStartedAt: 0, timers: new Set() }; }
  function selectFootballZone(zone, direction = false) { const runtime = runtimeFor('football-penalties') || createFootballRuntime(); if (runtime.phase !== 'ready') return; if (direction) runtime.selectedDirection = Number(zone); else runtime.selectedZone = Number(zone); state.runtime = runtime; render(); }
  ...
  function handleGameAction(action, cell, zone) {
    if (action === 'aviator-start') return startAviatorRound(); if (action === 'chicken-step') return startChickenStep(); if (action === 'chicken-reset') return resetChicken(); if (action === 'apple-reveal') return revealApple(cell); if (action === 'apple-reset') { stopGameAnimation(); state.runtime = createAppleRuntime(); return render(); } if (action === 'mine-open') return openMine(cell); if (action === 'mine-reset') return resetMineFromControls(); if (action === 'football-zone') return selectFootballZone(zone); if (action === 'football-direction') return selectFootballZone(zone, true); if (action === 'football-run') return runFootball(); if (action === 'football-next') { const previous = runtimeFor('football-penalties') || {}; stopGameAnimation(); state.runtime = { ...createFootballRuntime(), role: previous.role || 'striker', selectedZone: previous.selectedZone || 3, selectedDirection: previous.selectedDirection || 3, stake: previous.stake || 100 }; return render(); } if (action === 'football-rules') return openRules();
  }
