(function () {
  ...
  function renderFootballStage() {
    ...
    return `<div class="visual-stage football-stage pipeline-stage ${runtime.phase !== 'ready' ? `is-${runtime.phase}` : ''}" data-runtime-game="football-penalties"><div class="goal-scene"><div class="goal-net"><div class="goal-grid" aria-hidden="true"></div><canvas class="scene-animation-canvas football-animation-canvas" data-animation-canvas="football" aria-hidden="true"></canvas><div class="goal-zones">${[1, 2, 3, 4, 5].map((zone) => `<button class="goal-zone ${chosen === zone ? 'is-selected' : ''}" type="button" data-game-action="${runtime.role === 'striker' ? 'football-zone' : 'football-direction'}" data-zone="${zone}" ${runtime.phase !== 'ready' ? 'disabled' : ''} aria-label="${t('zone')} ${zone}"><span class="zone-number">${zone}</span><small class="zone-coefficient">${['1.42x','1.68x','2.00x','1.68x','1.42x'][zone - 1]}</small></button>`).join('')}</div><div class="keeper-actor" data-art-layer="keeper"><img class="keeper-sprite" src="${ART.football.goalkeeper}" alt="${t('goalkeeper')}"></div><div class="football-ball" data-art-layer="ball" aria-hidden="true"><img src="${ART.football.ball}" alt=""></div></div></div><div class="football-readout"><span class="readout-label">${t('result')}</span><strong data-football-result aria-live="polite">${runtime.result ? phaseLabel(runtime.result) : t('chooseZone')}</strong><small>${terminal && runtime.result === 'goal' ? `${t('simulatedReturn')}: 2x · ${formatMoney(runtime.stake || 100, { currencyFractionDigits: 0, currencyCode: 'RUB' })}` : t('virtualOnly')}</small></div><div class="stage-caption"><span>${runtime.role === 'striker' ? t('striker') : t('keeper')} / ${t('shotZones')}: 5</span><strong class="football-score">Счёт: ${runtime.score || 0}</strong></div></div>`;
  }
