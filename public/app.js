(function () {
  ...
  function langOptions() {
    const names = { en: 'English', ru: 'Русский', uk: '��країнська', pl: 'Polski', es: 'Español', pt: 'Português', de: 'Deutsch', fr: 'Français', it: 'Italiano', tr: 'Türkçe', ar: 'العربية' };
  ...
  function renderAppleGameView() {
    ...
    else if (runtime.phase === 'ended') statusText = runtime.score > 0 ? `${t('appleWin')}: ${runtime.score} \u20BD` : runtime.lostAtMultiplier ? `${t('appleLostAt')} x${String(runtime.lostAtMultiplier).replace(/^x/i, '')}` : t('appleBetPrompt');
    else {
  ...
  function revealApple(cell) { const runtime = runtimeFor('apple-of-fortune'); if (!runtime || runtime.phase !== 'safe') return; const rows = appleRows(); const row = rows.find((item) => item.level === runtime.activeRow) || rows[0]; runtime.revealed = { level: runtime.activeRow, cell: Number(cell), safe: Number(cell) === Number(row.recommendedCell) }; runtime.animationStartedAt = performance.now(); runtime.phase = 'opening'; state.runtime = runtime; render(); scheduleRuntime(() => { const wasSafe = runtime.revealed.safe; const multiplierValue = Number(String(row.multiplier || '1').replace(/^x/i, '')) || 1; runtime.revealedCells[runtime.revealed.level] = runtime.revealed; if (wasSafe) { runtime.score = Number((runtime.stake * multiplierValue).toFixed(2)); if (runtime.activeRow < rows.length) { runtime.activeRow += 1; runtime.phase = 'safe'; } else { runtime.phase = 'ended'; state.player.balanceMinor += toAppleMinor(runtime.score); } } else { runtime.score = 0; runtime.phase = 'ended'; runtime.lostAtMultiplier = row.multiplier; runtime.lostAtLevel = runtime.revealed.level; } runtime.revealed = null; state.runtime = runtime; render(); if (runtime.auto) { if (runtime.phase === 'safe') { const nextRow = rows.find((item) => item.level === runtime.activeRow); scheduleRuntime(() => revealApple(pickAppleAutoCell(nextRow)), 650); } else { scheduleRuntime(() => startAppleRound(), 1500); } } }, 560); }
  function pickAppleAutoCell(row) { if (!row) return 1; const failChance = Math.max(0, Math.min(0.85, 0.18 + (Number(row.level) || 1) * 0.03)); if (Math.random() < failChance) { const wrongCells = [1, 2, 3, 4, 5].filter((cell) => cell !== Number(row.recommendedCell)); return wrongCells[Math.floor(Math.random() * wrongCells.length)]; } return row.recommendedCell; }
